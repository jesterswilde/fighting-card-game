...
class Events extends Component<Props, State> {
  ...
  addEvent = () => {
    ...
    if (counter < this.props.events.length) {
      ...
      this.setState({
        cancelTimeout,
        events: [...events, this.props.events[counter]],
        counter: counter + 1
      });
    }
  };
    ...
    return (
      <div class="event-modal">
        <div
          ...
          onClick={dispatchFinishedDisplayingEvents}
        />
        <div class="event-container">
          {events.map((event, i) => {
            return (
              <div class="event" key={JSON.stringify(event) + i}>
                {this.reducer(event)}
              </div>
            );
          })}
  ...
  renderGameOver = (event: EventAction) => {
    ...
    if (event.winner < 0) {
      return (
        <div class="event-game-over opponent">
          Everybody Loses!
  ...
  renderAddedMechanic = (event: EventAction, opponent: boolean) => {
    return (
      <div class={`event-effect ${opponent ? "opponent" : ""}`}>
  ...
  renderRevealPrediction = (event: EventAction, opponent: boolean) => {
    ...
    return (
      <div class={`event-predict ${opponent ? "opponent" : ""}`}>
        <div>
          Prediction: {prediction}{" "}
          <div class="inline small">{correct ? "Correct" : "Incorrect"}</div>
        </div>
        <div class="changed">
          {"Correct Prediction(s): "}
          {correctGuesses.map((guess, i) => (
            <div key={i} class="inline">
              {" "}
              {guess}{" "}
            </div>
          ))}
  ...
  renderCard = (event: EventAction, opponent: boolean) => {
    return (
      <div class={`event-card ${opponent ? "opponent" : ""}`}>
        {" "}
        <div />
        <div>{event.cardName}</div> <div class="priority">{event.priority}</div>{" "}
      </div>
    );
  };
  renderEffect = (event: EventAction, opponent: boolean) => {
    ...
    const blocked = happenedTo.some(value => value === HappensEnum.BLOCKED)
      ? "blocked"
      : "";
    return (
      <div class={`event-effect ${opponent ? "opponent" : ""} ${blocked}`}>
        {mechanic !== undefined && mechanic}
        {mechanic !== undefined && " "}
        {player !== undefined && (
          <Arrow player={player} shouldFlip={opponent} />
        )}
  ...
  renderMechanic = (event: EventAction, opponent: boolean) => {
    return (
      <div class={`event-mechanic ${opponent ? "opponent" : ""}`}>
  ...
  renderEventSection = (eventSection: EventAction) => {
    return (
      <div class="event-section">
        {eventSection.gameEvents.map((playerEvent, playerIndex) => {
          return (
            <div class="players-events" key={playerIndex}>
              {playerEvent.gameEvents.map(event => {
                if (event === null) {
                  return <div />;
                }
  ...
  renderPredictionSection = ({ gameEvents: events = [] }: EventAction) => {
    return (
      <div class="event-section">
        {events.map((playerEvent, player) => {
          ...
          return (
            <div class="players-events" key={player}>
  ...
  renderCardNameSection = (eventSection: EventAction) => {
    return (
      <div class="event-section">
        {eventSection.gameEvents.map((cardEvent, playerIndex) => {
          if (cardEvent) {
            const opponent = cardEvent.playedBy !== this.props.player;
            return (
              <div class="card-names">
                {this.renderCard(cardEvent, opponent)}
              </div>
            );
          }
