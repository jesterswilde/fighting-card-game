...
import { getStylesToRequest } from "./util";
...
const getPossibleCards = async () => {
    ...
    const unfetchedStyles = getStylesToRequest(styles);
    if (unfetchedStyles.length > 0) {
        const fetched = await fetch(HOST_URL + '/decks/possibleCards?styles=' + unfetchedStyles.join(','), {
            method: "GET",
        });
        if (fetched.ok) {
